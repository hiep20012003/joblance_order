...
export class PaymentsService {
  ...
  async validate(
    ...
    const paymentIntent = await stripeClient.paymentIntents.retrieve(payment.transactionId!);
    const valid =
      payment.status === PaymentStatus.PENDING &&
            order.status === OrderStatus.PENDING && paymentIntent.status === 'requires_payment_method';
  ...
  async paymentSuccess(transactionId: string, status: PaymentStatus, metadata?: Record<string, unknown>): Promise<void> {
    return runInTransaction(await database.getConnection(), async (session) => {
      ...
      if (payment) {
        ...
        const order = await OrderModel.findOneAndUpdate({ _id: payment.orderId, status: OrderStatus.PENDING },
          {
            $set: {
